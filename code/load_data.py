...
import argparse
import pickle
from user_select import Selection
...
def train(instances, algorithm, online_training_iterations):
    # Train the model using "algorithm" on "data"
    # This is where new algorithms are added that subclass Predictor
    if algorithm.lower() == 'mc_perceptron':
        model = MulticlassPerceptron()
        
    model.train(instances, online_training_iterations)
...
def main():
    ...
    if args.mode.lower() == "train":
        ...
        # Train the model.
        predictor = train(instances, args.algorithm, args.online_training_iterations)
        try:
